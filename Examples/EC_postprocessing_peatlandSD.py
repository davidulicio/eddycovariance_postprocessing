...
"Eddy data"
path = 'D:/Dropbox/David/LECS/eddy/Turbera/Salida eddypro/2023/23-01-09/'
filename = 'eddypro_SDP_full_output_2023-01_adv.csv'
eddy_data = pd.read_csv(path+filename, low_memory=(False))
# eddy_units = eddy_data.iloc[0]; eddy_data = eddy_data[1:]
bool_repeated1 = eddy_data.duplicated(keep='first')
eddy_data = eddy_data[~bool_repeated1]  # Erase repeated data
date = pd.DatetimeIndex(eddy_data['date'].astype(str) + ' ' +
                        eddy_data['time'].astype(str))
eddy_data.index = date; eddy_data = eddy_data.astype(float, errors='ignore').resample('30min').mean()
date = eddy_data.index
"Biomet data"
filename = 'eddypro_SDP_biomet_2023-01_adv.csv'
...
biomet_data.index = date_biomet
biomet_data = biomet_data.drop(columns=['date', 'time'])
biomet_data = biomet_data.astype(float).resample('30min').mean()
date_biomet = biomet_data.index
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')

#%%
# for i in range(1,len(biomet_data['date'])+1):
#     if biomet_data['date'][i][0]!='2':
#         print('hay')
#%%
...
co2_flux[co2_flux<-2000] = np.nan
# plt.plot(date, co2_flux)
# plt.figure(figsize=(8,6))
# plt.plot(date[30000:60000], co2_flux[30000:60000])
...
longgap = 30 # Avoid extrapolation in gaps longer than longgap days

#%% Cut data to fit the shortest period
# Check every date of the datasets to determine the shortest period
...
biomet_data = biomet_data[~date_biomet.duplicated(keep='first')]
date_biomet = date_biomet[~date_biomet.duplicated(keep='first')]
#%% functions
...
def footprint(FETCHx, WD):
    ...
    a : List
        Boolean list where "0" comes from the North part of the peatland. Whereas
        "1" comes from South part of the SDP.

    """
    newangles = np.array([0, 18.8, 41, 57.7, 72, 
                          82.6, 101.1, 132, 144.7, 158.1, 173.9, 177.4, 187.3,
                          193, 217, 236.9, 251.4, 271.4, 284.6, 306.2, 312, 326.6,
                          341.7, 361])
    distnew = np.array([241.6, 220.3, 181, 160, 146,
                        139.3, 138.5, 122.2, 121.5, 147.2, 182, 205.5, 223, 143,
                        187, 228.6, 252.4, 253.8, 272.4, 335, 556, 478.4, 341.7, 241.6])
    erase = np.zeros_like(WD)
    loc = np.zeros_like(WD)
    for i in range(len(WD)):
        a = WD[i]
        if (82.6 < a < 271.4): loc[i] = 1
        if ~np.isnan(a):
            try:
...
def ml_gapfilling(df1, df2, method, test_size):
    ...
    df3 = df2.copy()
    df3 = df3.resample('30min').mean()
    df3[df3.isna()] = prediction[df3.isna().index]
    # Figure
...
def obukhov_length(T, ustar, H, rho):
    cp = 1012  # Specific heat of air [J kg−1 K−1].
    kappa = 0.41  # Von Karman constant 
    g = 9.81  # [m/s2]
    L = -(rho* cp * ustar * T) / (kappa * g * H)
    return L
...
#%% Biomet Variables
Ta = var_reading('Ta', biomet_data, True) -273.15
...
Ts1 = var_reading('Ts_1_1_1', biomet_data, True) - 273.15
Ts2 = var_reading('Ts_2_1_1', biomet_data, True) - 273.15
Ts3 = var_reading('Ts_3_1_1', biomet_data, True) - 273.15
...
FILENAME = 'peatland_extended_biomet_201401_202301.csv'
biomext = pd.read_csv(PATH+FILENAME)
date_biometext = pd.DatetimeIndex(biomext['TIMESTAMP_1'].astype(str)+'/'+
                biomext['TIMESTAMP_2'].astype(str) + '/' +
                biomext['TIMESTAMP_3'].astype(str) + ' ' +
                biomext['TIMESTAMP_4'].astype(str) + ':' +
                biomext['TIMESTAMP_5'].astype(str))
WTD = var_reading('WTD', biomext, True)
WTD = pd.Series(WTD, index=date_biometext).resample('30min').mean()
NR01TK = var_reading('NR01TK', biomext, True)
NR01TK = pd.Series(NR01TK, index=date_biometext).resample('30min').mean()
NR01TK[NR01TK>310.15] = np.nan
del biomext, date_biometext
WTD = WTD[1:np.where(WTD.index==date[len(date)-1])[0][0]]
...
ET = var_reading('ET', eddy_data, True) / 2  # 1hr agg to 30 min agg
...
# Atm. Pressure
window = (pd.Timestamp('2016-12-01 00:00:00') < date_biomet) & (date_biomet < pd.Timestamp('2017-08-15 16:30:00'))
normal_window = (pd.Timestamp('2014-01-01 00:00:00') < date_biomet) & (date_biomet < pd.Timestamp('2016-12-31 16:30:00'))
difference = -np.abs(np.nanmean(Pa[window]) - np.nanmean(Pa[normal_window]))
Pa_qc = dropped(Pa, window, difference)
#
window = (pd.Timestamp('2019-09-01 00:00:00') < date_biomet) & (date_biomet < pd.Timestamp('2020-03-31 16:00:00'))
difference = -np.abs(np.nanmean(Pa[window]) - np.nanmean(Pa[normal_window]))
Pa_qc = dropped(Pa, window, difference)
#
window = (date_biomet > pd.Timestamp('2020-09-10 16:00:00'))
difference = -np.abs(np.nanmean(Pa[window]) - np.nanmean(Pa[normal_window]))
Pa_qc = dropped(Pa, window, difference)
Pa_qc = quality_screening(Pa, 94000, 105000, 0, 0, 0)
...
WTD_qc = quality_screening(WTD, -.5, 1, 0, 0, 0)
#%% QS - eddy data
...
co2_strg_qc = quality_screening(co2_strg, -20, 20, 0, [co2_var, h2o_var, w_var, ts_var], qc_FCO2)
...
NEE_qc = quality_screening(NEE_qc, -25, 25, 0, [co2_var, h2o_var, w_var, ts_var],
                           qc_FCO2)
...
v_var = var_reading('v_var', eddy_data, True)
# v_var_QC = quality_screening(v_var, 0, np.nanpercentile(v_var, 0.999), 0, 0, 0)
...
WTD_o = biomet_gap_fill(WTD_qc, 0, WTD.index, date_era5, 'none')
...
NR01TK_o = biomet_gap_fill(NR01TK_qc, 0, NR01TK.index, date_era5, 'none')
...
v_var_o = biomet_gap_fill(v_var, 0, date, date_era5, 'none')
sigma_o = np.sqrt(v_var_o)
#%%
...
ft90, ns90 = footprint(FETCH_90, WD2)
ft70, ns70 = footprint(FETCH_70, WD2)
ftmax, nsmax = footprint(FETCH_MAX, WD2)
# data screening using footprint analysis
NEE_o[ft90==1] = np.nan
ET_o[ft90==1] = np.nan
LE_o[ft90==1] = np.nan
ns90 = pd.Series(np.array(ns90), index = WD2.index)
#%% Biomet Gapfilling
...
NR01TK_f = biomet_gap_fill(NR01TK_o, ds['t2m'], date_biomet, date_era5, 'linear fill')
...
print('....................................')
df = pd.DataFrame({'NEE': NEE_f, 'USTAR':ustar_o, 'TA':Ta_f}, index=date_era5)
mask = df.index.year<2023
df = df[mask]  # full years requierement
dff = df.copy(deep=True)  # Flag
dff[:] = 0
df[df.isna()] = undef; dff[df.isna()] = 2;
isday = SWin_f > 10; isday = isday[SWin_f.index.year<2023]  # full years requirement
...
NEE_o[NEE_o.index.year!=2023][flag==2] = np.nan
...
df = pd.DataFrame({'H': H_f, 'LE': LE_f, 'NEE': NEE_o, 'CO2_strg': co2_strg_o,
                   'SW_IN': SWin_f,'TA':Ta_f, 'VPD':VPD_f, 'ustar': ustar_o, 
                   'SHF1':SHF1_o, 'SHF2':SHF2_o, 'SHF3': SHF3_o}
                  , index=date_era5)
dff = df.copy(deep=True)  # Flag
dff[:] = 0
...
SHF1_f = pd.Series(var_reading('f_SHF1', df_f, True), index=SHF1_o.index)
SHF2_f = pd.Series(var_reading('f_SHF2', df_f, True), index=SHF2_o.index)
SHF3_f = pd.Series(var_reading('f_SHF3', df_f, True), index=SHF3_o.index)
ustar_f = var_reading('f_ustar', df_f, True)
CO2_strg_f = var_reading('f_CO2_strg', df_f, True)
# plt.plot(date_era5, NEE_f)
SHF = pd.concat([SHF1_f, SHF2_f, SHF3_f], axis=1).mean(axis=1)
SWC = pd.concat([SWC1_f, SWC2_f, SWC3_f], axis=1).mean(axis=1)
TS = pd.concat([Ts1_f, Ts2_f, Ts3_f], axis=1).mean(axis=1)
H_f = pd.Series(H_f, index=H_o.index)
#%% ML filling
#%% ML filling
X = pd.concat([SWin_f, MWS_f, LWout_f, TS, Rn_f, Ta_f, H_f], axis=1)
nee_f2 = pd.Series(NEE_f, index=NEE_o.index).copy()
nee_pred = ml_gapfilling(X, NEE_o, 'bagging', 0.2)
nee_f2[nee_f2.isna()] = nee_pred[nee_f2.isna()]
fco2_pred = ml_gapfilling(X, FCO2_o, 'bagging', 0.2)
X = pd.concat([SWin_f, MWS_f, LWout_f, TS, Rn_f, Ta_f, SWC, RH_f, Pa_f], axis=1)
et_f2 = ET_o.copy()
et_pred = ml_gapfilling(X, ET_o, 'bagging', 0.2)
et_f2[et_f2.isna()] = et_pred[et_f2.isna()]
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
...
print('....................................')
# DataFrame
df = pd.DataFrame({'NEE': nee_f2, 'SW_IN':SWin_f, 'TA':Ta_f, 'VPD':VPD_f})
df[df.isna()] = undef
# Flag
...
#%% ET filling
ET_f3 = LE_to_ET(LE_f, Ta_f) * 1800  # s -> 30 min
ET_f = ET_o.copy()
ET_f[ET_o.isna()] = ET_f3[ET_o.isna()]
ET_f[ET_f < 0] = 0
# plt.plot(ET_f)
#%% Monin-Obukhov Length
rho = air_density(Pa_f, Ta_f)
MO_LENGTH = obukhov_length(Ta_f, ustar_f, H_f, rho)
MO_LENGTH[(ustar_f<0.1)&(np.isfinite(ustar_f))] = np.nan
#%% Data saving
df_final = pd.DataFrame({'TIMESTAMP': date_era5, 'TA': Ta_o, 'TA_F': Ta_f,
                         'T_SONIC': sonic_temperature_o, 'CO2_mixing_ratio':co2_mix_ratio_o,
                         'H2O_mixing_ratio': h2o_mix_ratio_o, 'MO_LENGTH': MO_LENGTH,
                         'SC':co2_strg_o, 'SC_F': CO2_strg_f,
                         'FH2O': h2o_flux_o, 'FC': FCO2_o,'FC_F2': fco2_pred,
                         ...
                         'WS_MAX': MWS_o, 'WS_MAX_F':MWS_f,'V_SIGMA':sigma_o,
                         ...
                         'H_F': H_f, 'NEE': NEE_o, 'NEE_F': NEE_f, 'NEE_F2':nee_f2,
                         'NEE_err': NEE_err,'VPD': VPD_o, 'VPD_F': VPD_f,
                         'USTAR': ustar_o,  'USTAR_F': ustar_f, 'GPP_DT': gpp_l,
                         'GPP_NT': gpp_r, 'ET': ET_o, 'ET_F': ET_f, 'ET_F2': et_f2,
                         'RECO_DT': reco_l, 'RECO_NT':reco_r, 'Flux_direction': ns90})

df_final = df_final.round(4)
path = 'D:/Dropbox/David/LECS/eddy/Turbera/Salida PostProc/'
df_final.to_csv(path+'LEVEL3_SDP_2014_2022.csv', index=False)
