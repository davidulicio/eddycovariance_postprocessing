...
NEE = var_reading('co2_flux', eddy_data, True)
qc_NEE = var_reading('qc_co2_flux', eddy_data, True)
...
LE_qc = quality_screening(LE, -300, 650, 0, [h2o_var, w_var, ts_var, flowrate],
                          qc_LE)
NEE_qc = quality_screening(NEE, -100, 400, 0, [co2_var, h2o_var, w_var, ts_var],
                           qc_NEE)
...
df_final = pd.DataFrame({'TIMESTAMP': date_era5, 'Ta': Ta_o, 'Ta_f': Ta_f, 
                         ...
                         'P_rain': P_rain_o, 'MWS': MWS_o, 'WD': WD_o, 'Ts': Ts_o,
                         'SWC': SWC_o, 'SHF': SHF_o, 'LE': LE_o, 'H': H_o})
